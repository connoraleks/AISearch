...
import { DrawMode, Grid as GridInterface, Node as NodeInterface, NodeType } from './types';
...
const getNeighbors = (node: NodeInterface, grid: NodeInterface[][]) => {
...
export const getGridSize = (size: number, windowRef: React.RefObject<HTMLDivElement>) : {numRows: number, numCols: number} => {
    //get the size of the window accounting for 10% padding given the intended size of each node
    const numCols = Math.floor((windowRef.current?.clientWidth || 0) / size);
    const numRows = Math.floor((windowRef.current?.clientHeight || 0) / size);
    return {numRows, numCols};
};
// Returns the distance between the two given nodes calculated using the Pythagorean theorem
const getDistance = (nodeA: NodeInterface, nodeB: NodeInterface) => {
...
const getPath = (end: NodeInterface) => {
    let nodesInShortestPathOrder: NodeInterface[] = [];
    let currentNode = end;
    while(currentNode) {
...
const createGrid = (numRows: number, numCols: number, size: number) => {
...
const bfs = (grid: GridInterface) => {
    ...
    const visitedNodesInOrder: NodeInterface[] = [];
    const queue: NodeInterface[] = [];
    queue.push(startNode);
    while(queue.length) {
        const currentNode = queue.shift()!;
        currentNode.type = 'visited' as NodeType
        ...
        for(const neighbor of neighbors) {
            if(!(neighbor.type === 'visited') && !queue.includes(neighbor)) {
                queue.push(neighbor);
...
const dfs = (grid: GridInterface) => {
    ...
    while(stack.length) {
        ...
        const neighbors = getNeighbors(currentNode, gridCopy);
        for(const neighbor of neighbors) {
            if(!(neighbor.type === 'visited') && !stack.includes(neighbor)) {
                stack.push(neighbor);
                neighbor.parent = currentNode;
                // Also set the parent on the original grid, so that the shortest path can be found
                grid.nodes[neighbor.row][neighbor.col].parent = grid.nodes[currentNode.row][currentNode.col];
...
const dijkstra = (grid: GridInterface) => {
    ...
    if(grid.start === null || grid.end === null || grid.start === grid.end) return false;
    //Make a copy of the grid to perform the search on
    const gridCopy = grid.nodes.map(row => row.map(node => node.type === 'start' ? {...node, g: 0} : {...node, g: Infinity}));
    const startNode = gridCopy[grid.start.row][grid.start.col];
    const endNode = gridCopy[grid.end.row][grid.end.col];

    //Perform the search on the copy array of nodes, and return the visited nodes in order, while marking the parents of each node for the shortest path
    const visitedNodesInOrder: NodeInterface[] = [];
    const unvisitedNodes: NodeInterface[] = [];
...
//A* search
const aStar = (grid: GridInterface) => {
    if(grid.start === null || grid.end === null || grid.start === grid.end) return false;
    //Make a copy of the grid to perform the search on
    const gridCopy = grid.nodes.map(row => row.map(node => ({...node})));
    const startNode = gridCopy[grid.start.row][grid.start.col];
    ...
    while(unvisitedNodes.length) {
        ...
        const neighbors = getNeighbors(currentNode, gridCopy);
        for(const neighbor of neighbors) {
            if(!(neighbor.type === 'visited') && !unvisitedNodes.includes(neighbor)) {
...
const recursiveDivision = (grid: GridInterface, gap: number) => {
        ...
        if(rowEnd - rowStart > colEnd - colStart) {
            for(let i = colStart; i <= colEnd; i++) {
                if(!(i >= col  && i < col + gap) && gridCopy[row][i] !== startNode && gridCopy[row][i] !== endNode) {
                    gridCopy[row][i].type = 'wall' as NodeType
        ...
        // If the grid is wider than it is tall, create a random vertical wall with a gap
            ...
            for(let i = rowStart; i <= rowEnd; i++) {
                if(!(i >= row  && i < row + gap) && gridCopy[i][col] !== startNode && gridCopy[i][col] !== endNode) {
                    gridCopy[i][col].type = 'wall' as NodeType
...
//Prim's Algorithm
const prims = (grid: GridInterface) => {
    //TODO: Implement Prim's algorithm
    return false;
};
//Kruskal's Algorithm
const kruskals = (grid: GridInterface) => {
